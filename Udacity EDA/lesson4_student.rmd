...
ggplot(aes(x = age, y = friend_count), data = pf) +
  ...
  coord_cartesian(xlim = c(13,40), ylim = c(0,1000))
```
...
#0.3 is correlated but small, .5 is moderate, .7 is strong
...
#inferential statistics on experimental data addresses causality 
#descriptive statistics like our current can only address correlation and not causality
```
...
ggplot(aes(x = www_likes_received, y = likes_received), data = pf) + 
  ...
  scale_x_continuous(limits = c(0,quantile(pf$www_likes_received, .95))) + 
  scale_y_continuous(limits = c(0,quantile(pf$likes_received, .95))) + 
...
p2 <- ggplot(aes(x = age, y = friend_count_median), data = subset(pf.fc_by_age, age < 71)) +
  geom_line() +
  geom_smooth()
